...
export const floorFormatter = (n: number): string => {
  ...
  if (n >= 0.001 && n < 1) {
    return `${parseFloat(n.toFixed(3)).toLocaleString('en-US', {
  ...
  return `${Number(n.toFixed(2)).toLocaleString('en-US', { minimumFractionDigits: 2 })}`
}
...
export const volumeFormatter = (n: number): string => {
  ...
  return `${Number(n.toFixed(1)).toLocaleString('en-US', { minimumFractionDigits: 1 })}`
}
